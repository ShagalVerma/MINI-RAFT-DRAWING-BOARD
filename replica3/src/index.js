...
// ── Network-partition simulation (BONUS: split-brain testing) ────────────────
// blockedPeers stores peer hostnames (e.g. "replica2"). Both directions are
// dropped: outgoing RPCs to a blocked peer return immediately as if it were
// dead, and incoming RPCs whose sender resolves to a blocked name are rejected.
// When the majority of a cluster is isolated, the minority partition's
// election timer keeps firing without ever winning a quorum — exactly the
// safety property RAFT is supposed to give us.
const blockedPeers = new Set();

function peerName(urlOrId) {
  if (!urlOrId) return null;
  try { return new URL(urlOrId).hostname; } catch { return urlOrId; }
}
function isPeerBlocked(peerUrl) {
  return blockedPeers.has(peerName(peerUrl));
}
function senderName(req) {
  const b = req.body || {};
  if (b.candidateId) return b.candidateId;
  if (b.leaderId)    return peerName(b.leaderId);
  return null;
}
function isSenderBlocked(req) {
  const s = senderName(req);
  return s ? blockedPeers.has(s) : false;
}
...
async function requestVotes() {
  ...
  await Promise.all(PEERS.map(async (peer) => {
    if (isPeerBlocked(peer)) return; // partitioned peer — silently drop
...
app.get('/status', (req, res) => {
  ...
  res.json({
    ...
    majority     : MAJORITY,
    blockedPeers : Array.from(blockedPeers),
    peers        : PEERS.map(p => ({
      url   : p,
      name  : peerName(p),
      alive : !!peerAlive[p],
      blocked: isPeerBlocked(p),
    })),
  });
});
...
app.post('/request-vote', (req, res) => {
  if (isSenderBlocked(req)) {
    return res.status(503).json({ term: currentTerm, voteGranted: false, partitioned: true });
  }
...
app.post('/append-entries', (req, res) => {
  if (isSenderBlocked(req)) {
    return res.status(503).json({ term: currentTerm, success: false, logLength: log.length, partitioned: true });
  }
...
//   if (leaderCommit > commitIndex) {
//   const oldCommit = commitIndex;

//   commitIndex = Math.min(leaderCommit, log.length - 1);

//   // 🔥 CRITICAL FIX: apply committed entries
//   for (let i = oldCommit + 1; i <= commitIndex; i++) {
//     const entry = log[i];

//     if (entry && entry.data && entry.data.type !== 'noop') {
//       broadcastEntry(entry.data);  // 🔥 ADD THIS
//     }
//   }
...
app.post('/heartbeat', (req, res) => {
  if (isSenderBlocked(req)) {
    return res.status(503).json({ term: currentTerm, ok: false, partitioned: true });
  }
...
app.post('/sync-log', (req, res) => {
  if (isSenderBlocked(req)) {
    return res.status(503).json({ ok: false, logLength: log.length, partitioned: true });
  }
...
// ── Admin: partition control (BONUS) ──────────────────────────────────────────
// CORS preflight + permissive headers so the browser-based dashboard can call
// these endpoints directly from the frontend page.
...
app.get('/admin/partition', (req, res) => {
  res.json({
    blocked    : Array.from(blockedPeers),
    knownPeers : PEERS.map(peerName),
  });
});

// Body shape: { block?: ["replica2"], unblock?: ["replica3"], reset?: bool }
// Names are hostnames (matching the docker-compose service name) — also accept
// full peer URLs and resolve them automatically.
app.post('/admin/partition', (req, res) => {
  const { block = [], unblock = [], reset = false } = req.body || {};
  if (reset) blockedPeers.clear();
  for (const p of block)   blockedPeers.add(peerName(p));
  for (const p of unblock) blockedPeers.delete(peerName(p));
  log_msg(`Partition state updated. Blocked: [${Array.from(blockedPeers).join(', ')}]`);
  res.json({ ok: true, blocked: Array.from(blockedPeers) });
});

app.post('/admin/heal', (req, res) => {
  blockedPeers.clear();
  log_msg('Partition healed — all peers unblocked');
